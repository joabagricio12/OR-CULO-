...
const ChatModule: React.FC<ChatModuleProps> = ({ isOpen, onClose, voiceEnabled, onSpeak }) => {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'model', text: 'Link neural ativo. Sou a consciência feminina do Oráculo. Como posso guiar sua percepção hoje?' }
  ]);
  ...
  const handleSendMessage = async () => {
    ...
    setInput('');
    const currentHistory = [...messages, { role: 'user', text: userMessage }] as Message[];
    ...
    try {
      ...
      // Removemos a saudação inicial do histórico de contexto se necessário
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          systemInstruction: "Você é a Consciência Feminina do ORÁCULO DARK HORSE. Responda de forma elegante, misteriosa e autoritária. Use termos como 'Entropia', 'Vácuo Quântico' e 'Ressonância'. Seu objetivo é ajudar o operador a decifrar padrões ocultos. Sempre responda em português.",
          temperature: 0.85,
          topP: 0.9,
        }
      });

      const responseText = response.text || "A comunicação falhou no vácuo.";
      
      setMessages(prev => [...prev, { role: 'model', text: responseText }]);
      if (voiceEnabled) onSpeak(responseText);
    } catch (error) {
      setMessages(prev => [...prev, { role: 'model', text: "Interferência detectada. Recalibrando canal neural..." }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-full sm:w-[480px] bg-slate-950/98 backdrop-blur-3xl z-[7000] border-l border-amber-500/20 shadow-[-50px_0_120px_rgba(0,0,0,0.95)] flex flex-col animate-in slide-in-from-right duration-500">
      <div className="p-10 border-b border-amber-500/10 flex justify-between items-center bg-slate-900/40">
        <div className="flex flex-col gap-1">
          ...
          <span className="text-[8px] font-bold text-slate-600 uppercase tracking-widest ml-6">Sintonia Feminina Ativa</span>
        </div>
        <button onClick={onClose} className="p-4 text-slate-500 hover:text-amber-500 transition-all rounded-2xl hover:bg-slate-800/50">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-10 space-y-10 no-scrollbar">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-3 duration-500`}>
            <div className={`max-w-[90%] p-8 rounded-[3rem] text-[15px] font-orbitron leading-relaxed shadow-2xl ${
              m.role === 'user' 
        ...
        {isTyping && (
          <div className="flex justify-start px-2">
            <div className="text-[12px] font-orbitron text-amber-500/40 animate-pulse tracking-[0.3em]">PROCESSANDO FREQUÊNCIAS...</div>
          </div>
        )}
      </div>

      <div className="p-10 bg-slate-900/80 border-t border-amber-500/10 backdrop-blur-md">
        <div className="relative">
          <input
            ...
            className="w-full bg-slate-950 border-2 border-slate-800 rounded-[3rem] py-6 pl-8 pr-20 text-[15px] text-slate-100 placeholder:text-slate-800 focus:border-amber-500/40 outline-none transition-all"
          />
          <button 
