...
const App: React.FC = () => {
    ...
    const processAutoCorrection = (actualValues: string[]) => {
        ...
        if (newHits.some(h => h.status === 'Acerto')) {
            speak("Ressonância absoluta detectada.");
        } else if (newHits.some(h => h.status === 'Quase Acerto')) {
            speak("Aproximação por permutação identificada.");
        } else {
            speak("Dados assimilados. Calibrando.");
        }
    };
    ...
    return (
        <div className="min-h-screen bg-[#010409] px-4 pt-4 pb-16 gap-6 text-slate-100 flex flex-col overflow-y-auto no-scrollbar selection:bg-amber-500/30 font-orbitron hologram-noise relative max-w-7xl mx-auto">
            ...
            <div className="bg-slate-900/40 p-8 rounded-[2.5rem] border border-amber-500/10 shadow-2xl backdrop-blur-3xl oracle-glow relative overflow-hidden shrink-0">
                <div className="absolute top-0 right-0 w-64 h-64 bg-amber-500/5 blur-[120px] -mr-32 -mt-32"></div>
                <div className="flex justify-between mb-6 items-center">
                    <div className="flex flex-col">
                        <span className="text-[8px] text-slate-500 font-black uppercase tracking-[0.5em]">Entropia</span>
                        <span className="text-[16px] text-amber-500 font-black uppercase tracking-widest">Nível: {(settings.entropy * 100).toFixed(0)}%</span>
                    </div>
                    <button onClick={() => setSettings({...settings, voiceEnabled: !settings.voiceEnabled})} className={`p-5 rounded-[2rem] border transition-all active:scale-90 ${settings.voiceEnabled ? 'bg-amber-500/15 border-amber-500/40 text-amber-400' : 'bg-slate-800 border-slate-700 text-slate-600'}`}>
            ...
            <button onClick={handleGenerate} disabled={isLoading || isLocked} className={`w-full py-10 font-black rounded-[3.5rem] uppercase tracking-[0.6em] border-2 transition-all text-[14px] relative overflow-hidden group shrink-0 ${isLoading || isLocked ? 'bg-slate-900/50 border-slate-800 text-slate-700' : 'bg-slate-950 border-amber-600 text-amber-500 shadow-[0_0_60px_rgba(245,158,11,0.25)] active:scale-95'}`}>
                ...
                {isLoading ? 'ANALISANDO MATRIZ...' : isLocked ? 'MATRIZ MANIFESTADA' : 'INICIAR COLAPSO'}
            </button>
